...
class AnalysisInfo(object):
    ...
    def countMissingTable(self, fs: str) -> None:
        self.missingTableCount += len(self.missingTableRegex.findall(fs))

    def countMissingFunction(self, fs: str) -> None:
        self.missingFunctionCount += len(self.missingFunctionRegex.findall(fs))

    def countPostgresConvertError(self, fs: str) -> None:
        self.postgresConvertErrorCount += len(self.postgresConvertErrorRegex.findall(fs)) if self.postgresConvertErrorRegex else 0

    def countGlobalVariableError(self, fs: str) -> None:
        self.globalVariableErrorCount += len(self.globalVariableErrorRegex.findall(fs)) if self.globalVariableErrorRegex else 0
...
class DuckDB(AnalysisInfo):
    ...
    postgresConvertErrorRegex = re.compile(".*::(?:.*[a-zA-Z].*(?:\n|$))+RESULT:\n\tERROR - Catalog Error: Type with name [^\n]* does not exist!$")
    globalVariableErrorRegex = re.compile("^	ERROR - Parser Error: syntax error at or near \"@.*")
...
for dbName, info in analysisInfo.items():
    ...
    print(f"MySQL Global Variable Errors: {info.postgresConvertErrorCount}\n")